...
fn expand_main(
    ...
    let (input_device_config, devices, processors, polling_controllers) = expand_input_device_config(keyboard_config);
    let matrix_and_keyboard = expand_matrix_and_keyboard_init(keyboard_config);
    let (controller_initializers, mut controllers) = expand_controller_init(keyboard_config, &item_mod);
    controllers.extend(polling_controllers);
